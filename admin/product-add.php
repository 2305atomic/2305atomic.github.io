        ...
        try {
            $stmt = getDBConnection()->prepare("
                INSERT INTO products (name, slug, description, short_description, price, sale_price, sku, 
                                    stock_quantity, category_id, image, status, featured, weight, dimensions) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            
            if ($stmt->execute([$name, $slug, $description, $short_description, $price, $sale_price, 
                              $sku, $stock_quantity, $category_id, $image, $status, $featured, $weight, $dimensions])) {
            ...
            $_SESSION['admin_error'] = 'Error adding product: ' . $e->getMessage();
        }
    }
}
...
document.getElementById('name').addEventListener('input', function() {
    ...
    const sku = name.toUpperCase().replace(/[^A-Z0-9]/g, '').substring(0, 8) + '001';
